...
//    }
    ...
    if (self.isAct == YES) {
        float youhuiValue = [self YouHuiValue];
        float realValue = self.order_info.realTax + self.order_info.sumprice +self.order_info.realYunFei - youhuiValue;
        NSString *attrPrice = [NSString stringWithFormat:@"<font size=\"16\"><color value=\"000000\">实付金额：</><color value=\"#FF4E25\">￥%.2f</></>",realValue];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 2*self.order_info.cart.count+1) { //最后一行
        ...
        if (indexPath.row == 0) {//商品金额
            ...
            if (self.isAct == YES) {
                float youhuiValue = [self YouHuiValue];
                cell.priceLabel.text = [NSString stringWithFormat:@"-￥%.2f",youhuiValue];
            }else{
                ...
                if (indexPath.row == 0 || indexPath.row == 5 || indexPath.row == 3) {//税费  小计 满减
                    ...
                    if (cart.canOpenYouHui) { //有优惠券要使用的情况
                        if (self.isAct == YES) {
                            cell.isBack = YES;
                            float youhuiValue = [self YouHuiValue];
                            cell.rightLabel.text = [NSString stringWithFormat:@"-￥%.2f",youhuiValue];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 2*self.order_info.cart.count + 1) {
        ...
        case 1:
        {
            ...
            if (indexPath.row == 5) {
                return 60;
            }if (indexPath.row == 2 && self.isAct == YES) {
                return 88*cart.yhqdata.count+44;
            }else if (indexPath.row == 2 && cart.canOpenYouHui && cart.openYouHui) {
                return 88*cart.yhqdata.count+44;
            }
...
- (float)YouHuiValue{
    ...
    NSString *valuestr0 = [userdefaults objectForKey:@"0"];
    NSString *valuestr1 = [userdefaults objectForKey:@"1"];
    NSString *valuestr2 = [userdefaults objectForKey:@"2"];
    NSString *valuestr3 = [userdefaults objectForKey:@"3"];
    NSString *valuestr4 = [userdefaults objectForKey:@"4"];
    NSString *str0 = nil;
    NSString *str1 = nil;
    NSString *str2 = nil;
    NSString *str3 = nil;
    NSString *str4 = nil;
    if (valuestr0.length > 0) {
        str0 = [valuestr0 substringFromIndex:1];
    }
    if (valuestr1.length > 0) {
        str1 = [valuestr1 substringFromIndex:1];
    }
    if (valuestr2.length > 0) {
        str2 = [valuestr2 substringFromIndex:1];
    }
    if (valuestr3.length > 0) {
        str3 = [valuestr3 substringFromIndex:1];
    }
    if (valuestr4.length > 0) {
        str4 = [valuestr4 substringFromIndex:1];
    }
    float youhuiValue = str0.floatValue + str1.floatValue +str2.floatValue+str3.floatValue+str4.floatValue;
...
- (MLAddressSelectViewController *)addVc{
    if (!_addVc) {
        ...
        _addVc.addressSelectBlock = ^(MLAddressListModel *address){ //返回后设置当前地址为默认地址 然后重新拉取数据
            NSUserDefaults *userdefaults = [[NSUserDefaults alloc]init];
            NSString *str = [userdefaults objectForKey:@"0"];
            NSLog(@"str===%@",str);
            if (isActuse == YES) {
                weakself.isAct = YES;
            }
