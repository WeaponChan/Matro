...
@interface MLPersonController ()<UITableViewDelegate,UITableViewDataSource>
{
    ...
    JSBadgeView *imaBadgeView;
    UIImageView *adimgeView;//广告图
...
- (void)loadThirdButtonsView{
    ...
    _thirdButtonBackView = [[UIView alloc]initWithFrame:CGRectMake(0, 182, SIZE_WIDTH, (310.0f/750.0f)*SIZE_WIDTH -(100.0f/750.0f)*SIZE_WIDTH)];
...
- (void)createDianWoButton{
    ...
    ADBackView = [[UIView alloc] initWithFrame:CGRectMake(0, 187+(310.0f/750.0f)*SIZE_WIDTH - (100.0f/750.0f)*SIZE_WIDTH, SIZE_WIDTH, (100.0f/750.0f)*SIZE_WIDTH)];
    adimgeView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, SIZE_WIDTH, (100.0f/750.0f)*SIZE_WIDTH)];
    
    UITapGestureRecognizer* singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(bannarButtonAction:)];
    [ADBackView addGestureRecognizer:singleTap];
    
    [ADBackView addSubview:adimgeView];
    
    [_backgroundScrollView addSubview:ADBackView];
    
    _thirderHeight = 192.0f+(310.0f/750.0f)*SIZE_WIDTH;

}

-(void)getADdata{
    NSString *url = [NSString stringWithFormat:@"%@/api.php?m=member&s=usercenter&method=display",ZHOULU_ML_BASE_URLString];
    [MLHttpManager get:url params:nil m:@"member" s:@"usercenter" success:^(id responseObject) {
        _dianWOButton.enabled = YES;
        NSDictionary * result = (NSDictionary *)responseObject;
        if ([result[@"code"] isEqual:@0]) {
            NSDictionary * dataDic = result[@"data"];
            NSArray *dataArr = dataDic[@"advertise"];
            adimgArr = dataArr;
           NSLog(@"adimgArr===>>>>%@",adimgArr);
            if (adimgArr && adimgArr.count > 0) {
                NSDictionary *ADdic = adimgArr[0];
                NSString *imaUrl = ADdic[@"imgurl"];
                [adimgeView sd_setImageWithURL:[NSURL URLWithString:imaUrl] placeholderImage:[UIImage imageNamed:@"icon_default"]];
            }else{
                [adimgeView setImage:[UIImage imageNamed:@"icon_default"]];
            }
...
- (void)bannarButtonAction:(UITapGestureRecognizer *)tap{
    
    NSLog(@"执行了个人中心的广告按钮 === %@",adimgArr);
    if (adimgArr.count > 0) {
        NSDictionary *ADdic = adimgArr[0];
        NSString * type = ADdic[@"ggtype"];
        NSString * sender = @"";
        NSString * title = @"";
        ...
        if ([type isEqualToString:@"5"]) {
            ...
            sender = ADdic[@"ggv"];
            vc.store_link = [NSString stringWithFormat:@"%@/store/index?sid=%@&uid=%@",DianPuURL_URLString,sender,phone];
            vc.uid = sender;
        ...
        if ([type isEqualToString:@"6"]) {
            ...
            if (loginid && ![@"" isEqualToString:loginid]) {
                [self hideZLMessageBtnAndSetingBtn];
                MLVipCardListViewController *vc = [[MLVipCardListViewController alloc]init];
                vc.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:vc animated:YES];
            }else{
                [self showError];
    ...
    for (int i = 0; i<2; i++) {
        for (int j = 0; j<3; j++) {
            ...
            if (k == 2) {
                ...
                imaBadgeView = [[JSBadgeView alloc]initWithParentView:imageview alignment:JSBadgeViewAlignmentTopRight];
                imaBadgeView.badgeText = @"●";
                [imaBadgeView setBadgeTextColor:[HFSUtility hexStringToColor:Main_textRedBackgroundColor]];
                [imaBadgeView setBadgeBackgroundColor:[UIColor clearColor]];
                label.text = @"会员卡";
...
- (void)foursButtonsAction:(UIButton * )sender{
    ...
    if (sender.tag == 103) {
        ...
        NSString * str = [userdefault objectForKey:kUSERDEFAULT_USERCARDNO];
        if(![str isEqualToString:@""] && str ) {
//            MNNMemberViewController *vc = [MNNMemberViewController new];
//            memberVC.hidesBottomBarWhenPushed = YES;
            MLVipCardListViewController *vc = [[MLVipCardListViewController alloc] init];
            vc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:vc animated:YES];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (loginid && ![@"" isEqualToString:loginid]) {
        ...
        [ADBackView setFrame:CGRectMake(0, 187+(310.0f/750.0f)*SIZE_WIDTH - (100.0f/750.0f)*SIZE_WIDTH, SIZE_WIDTH, (100.0f/750.0f)*SIZE_WIDTH)];
        ...
        [ADBackView setFrame:CGRectMake(0, 182, SIZE_WIDTH, (100.0f/750.0f)*SIZE_WIDTH)];
        [_fourButtonsBackView setFrame:CGRectMake(0, 187 + (100.0f/750.0f)*SIZE_WIDTH, SIZE_WIDTH, SIZE_WIDTH/3.0*2.0)];
...
-(void)getIsShengji{
    NSString *url = [NSString stringWithFormat:@"%@/api.php?m=member&s=admin_cardup&action=cardUp",MATROJP_BASE_URL];
    [MLHttpManager get:url params:nil m:@"member" s:@"admin_cardup" success:^(id responseObject) {
        NSDictionary *result = (NSDictionary *)responseObject;
        NSLog(@"result=11=33=%@",result);
        if ([result[@"code"] isEqual:@0]) {
            NSDictionary *data = result[@"data"][@"cardInfo"];
             NSString *upStatus = data[@"upStatus"];
            
            NSLog(@"upStatus===%@",upStatus);
                if (upStatus.intValue != 0) {
                    imaBadgeView.hidden = NO;
                    imaBadgeView.badgeText = @"●";
                    [imaBadgeView setBadgeTextFont:[UIFont systemFontOfSize:8]];
                }else{
                    imaBadgeView.hidden = YES;
