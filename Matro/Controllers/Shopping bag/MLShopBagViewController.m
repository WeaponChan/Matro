...
@property (nonatomic,strong)MLShopCarOutListModel *shopOutCart;
...
- (void)viewDidLoad {
    ...
    _tableView = ({
        ...
        [tableView registerNib:[UINib nibWithNibName:@"MLShopBagOutdateCell" bundle:nil] forCellReuseIdentifier:kShopBagOutadateViewCell];
        ...
        [tableView registerClass:[MLShopBagOutdateHeaderView class] forHeaderFooterViewReuseIdentifier:@"OutdateHeaderView"];
//        [tableView registerNib:[UINib nibWithNibName:@"MLShopBagOutdateHeaderView" bundle:nil] forHeaderFooterViewReuseIdentifier:@"OutdateHeaderView"];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    if (self.likeArray.count > 0 && (self.shopCart.cart.count > 0 || self.shopOutCart.cart.count > 0)) {
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
   ...
   return  self.isLogin?(self.shopCart.cart.count + self.shopOutCart.cart.count):self.offlineCart.count;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (self.isLogin) {
        if (section >= self.shopCart.cart.count && section <(self.shopCart.cart.count + self.shopOutCart.cart.count)) {
            MLShopingCartOutModel *cart = [self.shopOutCart.cart objectAtIndex:(section -self.shopCart.cart.count)];
            if (cart.isMore) {
                if (cart.isOpen) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.isLogin) {
        if (indexPath.section >= self.shopCart.cart.count && indexPath.section <(self.shopCart.cart.count + self.shopOutCart.cart.count)) {
            MLShopBagOutdateCell *cell = [tableView dequeueReusableCellWithIdentifier:kShopBagOutadateViewCell forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            __block  MLShopingCartOutModel *cart = [self.shopOutCart.cart objectAtIndex:(indexPath.section-self.shopCart.cart.count)];
            ...
            __block  MLProlistOutModel *model = [cart.prolist objectAtIndex:indexPath.row];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (self.isLogin) {
        if (section >= self.shopCart.cart.count && section <(self.shopCart.cart.count + self.shopOutCart.cart.count)) {
            MLShopBagOutdateHeaderView *cartHead = [tableView dequeueReusableHeaderFooterViewWithIdentifier:@"OutdateHeaderView"];
            if (!cartHead) {
                cartHead = [[MLShopBagOutdateHeaderView alloc]initWithReuseIdentifier:@"OutdateHeaderView"];
            }
            MLShopingCartOutModel *model = [self.shopOutCart.cart objectAtIndex:(section - self.shopCart.cart.count)];
            cartHead.shopingCart = model;
            cartHead.shopOutdateBlock = ^(){ //点击店铺事件
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (!self.isLogin) {
        ...
        if (indexPath.section >= self.shopCart.cart.count && indexPath.section <(self.shopCart.cart.count + self.shopOutCart.cart.count)) {
            MLShopingCartOutModel *cart = [self.shopOutCart.cart objectAtIndex:(indexPath.section -self.shopCart.cart.count) ];
            if (cart.isMore && cart.isOpen && indexPath.row == cart.prolist.count) {
                cart.isOpen = NO;
                [self.tableView reloadData];
                return;
            ...
            MLProlistOutModel *model = [cart.prolist objectAtIndex:indexPath.row];
 ...
 */
    [self.view configBlankPage:EaseBlankPageTypeGouWuDai hasData:(self.isLogin? (self.shopCart.cart.count>0 || self.shopOutCart.cart.count>0):self.offlineCart.count>0)];
...
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"back"] style:UIBarButtonItemStylePlain target:self action:@selector(backBtnAction)];
    ...
    [self.shopOutCart.cart removeAllObjects];
    [self.tableView reloadData];
    [self.collectionView reloadData];
    if (loginid) { //已登录情况
...
- (void)getDataSource{
    ...
    [MLHttpManager get:url params:nil m:@"product" s:@"cart" success:^(id responseObject) {
        ...
        if ([[result objectForKey:@"code"] isEqual:@0]) {
            ...
            self.shopOutCart = [MLShopCarOutListModel mj_objectWithKeyValues:result[@"data"][@"xia_cartlist"]];
            [self countAllPrice];
            [self.tableView reloadData];
            if (self.shopCart.cart.count > 0 || self.shopOutCart.cart.count > 0) {
                [self guessYourLike];
            }else{
                [self closeLoadingView];
                [self configBlankPage];
...
- (void)deleteOutdateGoods:(MLProlistOutModel *)goods{
    //[MBProgressHUD showHUDAddedTo:self.view animated:YES];
    [self showLoadingView];
    NSString *url = [NSString stringWithFormat:@"%@/api.php?m=product&s=cart&action=delete",MATROJP_BASE_URL];
    [MLHttpManager post:url params:@{@"cart_id":goods.ID} m:@"product" s:@"cart" success:^(id responseObject) {
