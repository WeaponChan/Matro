...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(getMessageDataSource) name:@"PUSHMESSAGEActive" object:nil];
    ...
    if (del.isShiXiao == YES) {
        [self loginAction:nil];
        [MBProgressHUD show:del.shixiaoMsg view:self.view];
    }
    if (del.pushMessage) {
...
- (void)pushMessage:(NSNotification *)not{
    ...
    switch (message.go) {
        ...
        case PushMessageGOProductDetail:
        {
            ...
            break;
        case PushMessageGODingdanDetail:
        {
        ...
        case PushMessageGOVipList:{
            ...
            break;
        case PushMessageGOVipDetail :{
            NSDictionary *dic = message.card_info;
            NSLog(@"dic ---->%@",dic);
            NSString *cardID = dic[@"cardID"];
