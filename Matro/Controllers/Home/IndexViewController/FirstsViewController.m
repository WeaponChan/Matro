...
- (void)viewDidLoad {
    ...
    adtitleArr = [[NSMutableArray alloc] init];
    fourTitleArr = [NSMutableArray array];
    activityDic = [NSDictionary dictionary];
            ...
            if ([result[@"data"] isKindOfClass:[NSDictionary class]]) {
             ...
             //1.0首页轮显
                ...
                //广告文字轮播
                    ...
                    if ([_dataZongDic[@"ggdata"] isKindOfClass:[NSArray class]]) {
                        [adtitleArr removeAllObjects];
                        NSArray *adArr = _dataZongDic[@"ggdata"];
                        if (adArr && adArr.count > 0) {
                                NSString *title = nil;
                                for (NSDictionary *adDic in adArr) {
                                    title = adDic[@"title"];
                                    [adtitleArr addObject:title];
                                }
                            }
                        }
...
- (void)loadIndex_1_view{
    ...
    self.fourButtonView.titleLab1.text = self.firstStr;
    ...
    self.fourButtonView.titleLab2.text = self.secondStr;
    ...
    self.fourButtonView.titleLab3.text = self.threeStr;
    ...
    self.fourButtonView.titleLab4.text = self.fourStr;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (collectionView.tag == 105) {
        ...
        //是否有参加的活动
        if (dics[@"activity_pic"]) {
            NSArray *activity_picArr = dics[@"activity_pic"];
            CGFloat width = 0.f;
...
//            cell.cuxiaoPriceLabel.text = [NSString stringWithFormat:@"￥%@",price];
        ...
        if (collectionView.tag == 101) {
            ...
            //是否有参加的活动
            if (goodDic[@"activity_pic"]) {
                NSArray *activity_picArr = goodDic[@"activity_pic"];
                CGFloat width = 0.f;
                if (activity_picArr && activity_picArr.count>0) {
                    cell.ActivityView.hidden = NO;
                    for (int i = 0; i<activity_picArr.count; i++) {
...
- (UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        ...
        case 1:{
            ...
            break;
        case 2:{
            static NSString *ADCellIdentifier = @"MLADlunboTableViewCell";
            MLADlunboTableViewCell *AdTableViewCell = [tableView dequeueReusableCellWithIdentifier:ADCellIdentifier];
            ...
            if ([_dataZongDic[@"ggimg"] isKindOfClass:[NSArray class]]) {
                NSArray *ggimgArr = _dataZongDic[@"ggimg"];
                if (ggimgArr && ggimgArr.count > 0) {
                    NSString *imgStr = ggimgArr[0][@"imgurl"];
                    if ([imgStr hasSuffix:@"webp"]) {
                        [AdTableViewCell.ADimg setZLWebPImageWithURLStr:imgStr withPlaceHolderImage:PLACEHOLDER_IMAGE];
                    } else {
                        [AdTableViewCell.ADimg sd_setImageWithURL:[NSURL URLWithString:imgStr] placeholderImage:[UIImage imageNamed:PlaceholderImage_Name]];
                    }
                   
                }else{
                    [AdTableViewCell.ADimg setImage:[UIImage imageNamed:@"icon_default"]];
                }
                
            }
             
            adWidth = AdTableViewCell.ADView.frame.size.width;
            adHeight = AdTableViewCell.ADView.frame.size.height;
            CCPScrollView *ccpView = [[CCPScrollView alloc] initWithFrame:CGRectMake(0, 0, adWidth, adHeight)];
            ccpView.titleFont = 20;
            ccpView.tintColor = [UIColor redColor];
            if (adtitleArr.count>0) {
                ccpView .titleArray  = adtitleArr;//[NSArray arrayWithObjects:@"1",@"2",@"3",@"4", nil]
            }
            
            [ccpView clickTitleLabel:^(NSInteger index,NSString *titleString){
                NSLog(@"index===%ld===%@",index,titleString);
                NSArray *ADarray = _dataZongDic[@"ggdata"];
                
                if (ADarray.count > 0) {
                    NSDictionary *ADdic = ADarray[index - 100];
            ...
            [AdTableViewCell.ADView addSubview:ccpView];
...
//-(void)repeadFuntion{
//    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, adHeight, adWidth, adHeight)];
//    if (self.ADindex == 1) {
...
//    
//    UILabel *labelOnChangeView = [self.changeview viewWithTag:self.ADindex];
//    
//    [UIView animateWithDuration:1 animations:^{
//        labelOnChangeView.frame = CGRectMake(0, -adHeight, adWidth, adHeight);
//        label.frame = CGRectMake(0, 0, adWidth, adHeight);
